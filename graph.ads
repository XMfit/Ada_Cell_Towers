...
package Graph is
   ...
   type Graph is tagged record
      Nodes : Ada.Containers.Double_Linked_Lists.List;  -- A list of nodes in the graph.
   end record;

   -- Initialize the graph.
   procedure Initialize(G : in out Graph);

   -- Check if a node with the given data exists in the graph.
   function Node_Exists(G : in out Graph; Data : in Node_Type) return Node_Access;

   -- Add a new node to the graph.
   procedure Add_Node(G : in out Graph; Data : in Node_Type);

   -- Add an edge between two nodes if they both exist.
   procedure Add_Edge(G : in out Graph; From : in Node_Type; To : in Node_Type);

   -- Delete an edge between two nodes.
   procedure Delete_Edge(G : in out Graph; From : in Node_Type; To : in Node_Type);

   -- Check if a path exists between two nodes in the graph.
   function Path_Exists(G : in Graph; From : in Node_Type; To : in Node_Type) return Boolean;
