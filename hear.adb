...
procedure hear is
   ...
   function Get_Name (Input : String; Start_Pos : Index;
                      End_Pos : Index) return String is
   ...
   function Return_Command (Input : String;
                            Start_Pos : Index) return Character is
   begin
      for I in Start_Pos .. 150 loop
   ...
   procedure Execute_Command (Tower_One : Unbounded_String;
                             Tower_Two : Unbounded_String;
                             Command : Character) is

   begin
      if Command = '.' then
         --   Check if towers exists in source, and add if need be
         ...
         if not Graph.Direct_Link (Tower_One, Tower_Two) then
         ...
         --   Connection doesn't exist if not in master list.
         if not (Graph.In_Master_List (Tower_One)
                and Graph.In_Master_List (Tower_Two))
         then
            Put_Line ("- " & To_String (Tower_One) & " => "
                      & To_String (Tower_Two));
         else
...
begin
   ...
   while not (End_Of_File) loop
      ...
      --   Stores names
      Tower_One_Name := To_Unbounded_String
                        (Get_Name (Input, Tower_One_Start, Tower_One_End));
      Tower_Two_Name := To_Unbounded_String
                        (Get_Name (Input, Tower_Two_Start, Tower_Two_End));
